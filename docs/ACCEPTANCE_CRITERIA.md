...
| AC-LOCALTRANSFER | Local Device Transfer | Creates a compressed WebRTC offer in a first QR; creates and accepts a return-answer QR without a signalling backend; displays the same six-digit pairing code on both devices; establishes an encrypted browser data channel using local ICE candidates; sends text up to 100,000 characters; asks the receiver to accept or decline each file up to 100 MB; transfers file chunks with progress and SHA-256 verification where Web Crypto is available; supports QR camera scanning, QR image upload, and manual code fallback; explains that guest Wi-Fi, VPNs, or client isolation can block peer connections; never uploads or stores transfer contents. |
