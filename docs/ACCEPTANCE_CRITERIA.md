...
| AC-TEXTQR | QR Text Transfer | Encodes short text or a tiny file into a same-site QR URL without a backend; opens and decodes the receive route locally; reconstructs file name, MIME type, bytes, preview, and download; uses gzip when it makes the payload smaller; rejects unsafe executable extensions, source files above 20 KB, and final QR URLs above the reliable capacity; supports copy, safe-link opening, QR download, camera scanning, and QR-image upload; warns above 1,200 text characters and blocks above 1,500. |
