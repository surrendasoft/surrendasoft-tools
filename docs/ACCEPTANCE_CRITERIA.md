...
| AC-LOCALTRANSFER | Local Device Transfer | Uses PeerJS Cloud for temporary third-party signalling and creates a short one-QR session link that automatically connects the receiving browser; discloses the provider and clearly distinguishes signalling metadata from peer-to-peer transfer contents; retains the animated offer/answer QR method as an advanced no-provider fallback; displays the same six-digit pairing code on both devices; establishes an encrypted WebRTC data channel; sends text up to 100,000 characters; asks the receiver to accept or decline each file up to 100 MB; transfers file chunks with progress and SHA-256 verification where Web Crypto is available; never uploads or stores transfer contents. |
