...
| AC-SIGNPDF | Fill & Sign PDF | Accepts and previews a PDF with crisp re-rendered zoom (50%–400%); lets the user drag to draw text boxes on flat forms with inline typing, move, and resize; accepts a drawn/uploaded signature with placement and resize; creates one downloadable completed PDF; validates when no text or signature has been added. |
