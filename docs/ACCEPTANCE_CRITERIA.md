...
| AC-LOCALTRANSFER | Local Device Transfer | Creates a compressed WebRTC offer in a first QR; creates and accepts a return-answer QR without a signalling backend; guides users through a sender/receiver pairing flow with paste-first return completion for laptop senders; displays the same six-digit pairing code on both devices; establishes an encrypted browser data channel using local ICE candidates; sends text up to 100,000 characters; asks the receiver to accept or decline each file up to 100 MB; transfers file chunks with progress and SHA-256 verification where Web Crypto is available; supports animated QR camera scanning with live hold-steady feedback, QR image upload, and manual code fallback; explains that guest Wi-Fi, VPNs, or client isolation can block peer connections; never uploads or stores transfer contents. |

## Test layers

- `toolContracts.test.jsx` mounts every one of the 40 direct tool routes and checks its primary workspace contract.
