...
function Home({ onOpen }) {
  ...
  return <>
    ...
    <section className="tools-section wrap" id="tools">
      ...
      {visibleTools.length ? <div className="tool-grid">{visibleTools.map(tool => <ToolCard key={tool.id} tool={tool} onOpen={onOpen}/>)}</div> : <div className="no-results"><span>💡</span><h3>No tools found</h3><p>Can't find what you need? Let us know what you want us to build!</p><div style={{display:'flex',gap:10,flexWrap:'wrap',justifyContent:'center'}}><button onClick={() => { setQuery(''); setFilter('All'); }}>Show all tools</button><button className="button primary" onClick={() => onOpen('suggest')}>Suggest a tool <Icon name="arrow" size={16}/></button></div></div>}
